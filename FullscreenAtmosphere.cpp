...
osg::Node* FullscreenAtmosphere::createAtmosphere(osg::Node* subgraph, osg::Camera* camera){
    ...
    ss->addUniform(new osg::Uniform("groundTexture", 2));    
...
void FullscreenAtmosphere::initShaders(osg::Camera* camera,osg::StateSet* ss)
{
    // 创建着色器程序
    osg::ref_ptr<osg::Program> program = new osg::Program;

    // 使用OSG的标准方法从文件读取着色器
    osg::ref_ptr<osg::Shader> vertexShader = osg::Shader::readShaderFile(osg::Shader::VERTEX, "e:/qwidget1/shader/cloud.vert");
    osg::ref_ptr<osg::Shader> fragmentShader = osg::Shader::readShaderFile(osg::Shader::FRAGMENT, "e:/qwidget1/shader/cloud.frag");
...
void FullscreenAtmosphere::initUniforms(osg::StateSet* ss)
{
    // 初始化uniform变量
...
void FullscreenAtmosphere::initializeCloudTextures(osg::StateSet* ss)
{
  ...
  // osg::ref_ptr<osg::Image> shapeNoiseImage = osgDB::readImageFile("E:/a.png");
    ...
    if (curlNoiseImage.valid()) {
        ...
        ss->addUniform(new osg::Uniform("_curlNoiseTex", 7));
...
osg::Camera* FullscreenAtmosphere::createRTTCamera(osg::Texture2D*& tex,  osg::Vec4 backColor)
{
    int width = 1024;
    int height = 1024;
...
void FullscreenAtmosphere::updateCloudParameters(float shapescale, float detailScale, float windSpeed,
...
void FullscreenAtmosphere::resetCloudParameters()
{
    // 重置云参数为默认值
    if (_shapescale.valid()) {
        _shapescale->set(0.0003f);
