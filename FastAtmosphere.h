#ifndef FAST_ATMOSPHERE_H
#define FAST_ATMOSPHERE_H
...
// FastTimeCallback回调类声明
class FastTimeCallback : public osg::UniformCallback
{
public:
    FastTimeCallback() : mCurrentTime(0.0f) {}
...
// FastCallback回调类声明
class FastCallback : public osg::StateSet::Callback
{
public:
    explicit FastCallback(osg::Camera* camera) : m_camera(camera), nFrame(0) {
        // 初始化鼠标位置
...
class FastAtmosphere : public osg::Geode
{
public:
    FastAtmosphere();
    virtual ~FastAtmosphere();
    ...
    // FastCallback引用
    osg::ref_ptr<FastCallback> _fastCallback;
};
