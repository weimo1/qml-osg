...
osg::ref_ptr<osg::Geode> FastAtmosphere::createFullScreenQuad(osg::Camera* camera){
...
osg::Node* FastAtmosphere::createAtmosphere(osg::Node* subgraph, osg::Camera* camera){
...
void FastAtmosphere::initShaders(osg::Camera* camera,osg::StateSet* ss)
{
    // 创建着色器程序
    ...
    _fastCallback = new FastCallback(camera);
    ss->setUpdateCallback(_fastCallback.get());
    initializeCloudTextures(ss);
   
}

void FastAtmosphere::initUniforms(osg::StateSet* ss)
{
    // 初始化uniform变量
    ...
    _time->setUpdateCallback(new FastTimeCallback());
}




void FastAtmosphere::initializeCloudTextures(osg::StateSet* ss)
{
    // 加载3D基础形状纹理
...
// FastCallback类的实现
void FastCallback::process(osg::StateSet* ss)
{
    nFrame++;
    if (nFrame > 6000)
        nFrame = 1;  // 重置为1而不是10
...
osg::Camera* FastAtmosphere::createHUDCamera(double left, double right, double bottom, double top)
{
...
osg::Camera* FastAtmosphere::createRTTCamera(osg::Texture2D*& tex,  osg::Vec4 backColor)
{
    int width = 1024;
    int height = 1024;
...
void FastAtmosphere::updateCloudParameters(float shapescale, float detailScale, float windSpeed,
...
void FastAtmosphere::resetCloudParameters()
{
    // 重置云参数为默认值
    if (_shapescale.valid()) {
        _shapescale->set(0.0003f);
...
void FastAtmosphere::updateMousePosition(float x, float y)
{
    // 直接更新FastCallback中的鼠标位置
    if (_fastCallback.valid()) {
        _fastCallback->setMousePosition(x, y);
    }
    
    // 同时更新iMouse uniform变量
    // if (_iMouse.valid()) {
    //     _iMouse->set(osg::Vec2(x, y));
    // }
}
