...
osg::Texture* AtmosphereDemo::createTexture(int format, const std::string& fileName, int nW, int nH, int nT)
{
    ...
    if (err != 0) {
        // 文件打开失败，尝试在项目目录中查找
        std::string projectPath = "E:/qwidget1/";
...
{
    // 读取着色器文件
    std::string strDir = "E:/qwidget1/shader/";
...
void AtmosphereDemo::initializeCloudTextures(osg::StateSet* ss)
{
    ...
    // 加载高度密度纹理，用于控制云的高度分布
    osg::ref_ptr<osg::Texture2D> heightDensityTexture = new osg::Texture2D;
    osg::ref_ptr<osg::Image> heightDensityImage = osgDB::readImageFile("E:/cloud1/HeightDensityMap.png");
    
    if (heightDensityImage.valid()) {
        heightDensityTexture->setImage(heightDensityImage);
        heightDensityTexture->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR);
        heightDensityTexture->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR);
        heightDensityTexture->setWrap(osg::Texture::WRAP_S, osg::Texture::REPEAT);
        heightDensityTexture->setWrap(osg::Texture::WRAP_T, osg::Texture::REPEAT);
        
        // 将高度密度纹理绑定到纹理单元11
        ss->setTextureAttributeAndModes(8, heightDensityTexture);
        ss->addUniform(new osg::Uniform("_HeightDensityMap", 8));
        
        printf("Height density texture loaded and configured successfully\n");
    } else {
        printf("Failed to load height density texture\n");
    }
    // 加载蓝噪声纹理，用于消除云渲染分层
